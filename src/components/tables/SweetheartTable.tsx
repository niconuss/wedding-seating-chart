...
import { SWEEP_WIDTH, SWEEP_HEIGHT } from '@/utils/constants';
...
interface SweetheartTableProps {
  table: Table;
  isEditing?: boolean;
  onEditEnd?: () => void;
...
const PAD = 40;
const W = SWEEP_WIDTH + PAD * 2;
const H = SWEEP_HEIGHT + PAD * 2;
const CX = W / 2;
const CY = H / 2;

export function SweetheartTable({ table, isEditing, onEditEnd, onDoubleClickName }: SweetheartTableProps) {
  return (
    <div style={{ width: W, height: H, position: 'relative' }}>
      ...
      >
        ...
        />
        {/* Hearts decoration */}
        <text x={CX} y={CY + 6} textAnchor="middle" fontSize="14" fill="#0d9488">
          ♥
        </text>
      </svg>

      {/* Name label — centered inside pill */}
      <div
        style={{
          position: 'absolute',
          left: CX,
      ...
      {table.seats.map((seat) => {
        const pos = seatPosition('sweetheart', seat.index, table.seats.length);
        ...
        return (
          <React.Fragment key={seat.index}>
            ...
            {seat.guestId && (
              <SeatedGuestChip guestId={seat.guestId} tableId={table.id} seatIndex={seat.index} style={style} labelAbove={true} />
            )}
          </React.Fragment>
        );
      })}
